...
function getShakeOffset(startedAt, duration, maxOffset, seed, currentTime) {
  ...
  const strength = maxOffset * (1 - progress) ** 2;
  return {
...
function normalizePlayerProgress(progress = {}) {
  ...
  const normalizedMastery = normalizeMasteryProgress(mastery);
  ...
  return {
    ...
    lifetime: {
      matches: Math.max(
        ...
        Number.parseInt(savedDaily.matches, 10) || 0,
      ),
      wins: Math.max(0, Number.parseInt(savedLifetime.wins, 10) || 0, Number.parseInt(savedDaily.wins, 10) || 0),
...
function drawResultInsight(x, y, width, maxHeight = Infinity, options = {}) {
  ...
  const maxReasonLines = Math.max(1, Math.min(compact ? 2 : 3, Math.floor((maxHeight - titleHeight - lessonLineHeight - statsLineHeight - 4) / reasonLineHeight)));
